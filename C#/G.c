...
const static struct I1Table {
    ...
    const unsigned int table[N_I1_METHODS];
} i1Table = {
    .id = I1,
    .table = {
        offsetof(struct GTableStruct, computeX) / sizeof(void *),
        offsetof(struct GTableStruct, update) / sizeof(void *),
        offsetof(struct GTableStruct, updateAndComputeX) / sizeof(void *)
    }
};

const static struct I2Table {
    const enum InterfaceId id;
    const unsigned int table[N_I2_METHODS];
} i2Table = {
    .id = I2,
    .table = {
        offsetof(struct GTableStruct, computeX) / sizeof(void *), 
